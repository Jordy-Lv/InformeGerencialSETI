# F7 — Adaptador Aranda (carga manual) + perfil Bancoldex

## Contexto

Bancoldex no comparte instancia GLPI con Acción Fiduciaria ni Novaventa: sus
...
reconocimiento completo con evidencia real. Bancoldex también difiere lo
...
registrados en `PERFILES_REGISTRADOS`. Bancoldex es el primer cliente que
...
4. `cargarCasosAranda()` — nuevo cargador que lee, valida y agrega los casos
   de Bancoldex por tipo, motor y SLA, sin tocar `cargarGlpi()` ni ninguna
...
Con Bancoldex como prioridad confirmada por el usuario, se completó en la
...
12. Preset final de Bancoldex: `c3, c4, c5, c7, c8, c8m`. Incluye únicamente
...
13. `c5` publica los agregados de Aranda en el dominio compartido `casos` y
    ...
    Fiduciaria. Los desgloses propios de Bancoldex se adaptan como gráficas de
...
**Actualización del 06/08/2026:** esta coordinación ya no es necesaria en el
sentido original. El usuario pidió completar Aranda/AlertsList para
Bancoldex mientras `codex/f6-perfil-novaventa` seguía siendo la rama activa
(con F6 aún sin cerrar), y `docs/2026-08-06-divergencia-bancoldex.md` ya
había mostrado que un merge completo de `codex/bancoldex-completo` traería
20 bloques en conflicto, 4 de ellos con decisión de diseño pendiente. Se
optó por reabrir este change y aplicarlo directamente sobre
`codex/f6-perfil-novaventa`, portando solo las piezas que esa rama no tenía
(ver `tasks.md`, "Alcance reducido"). `codex/bancoldex-completo` queda como
referencia histórica; no se mergea a `main` — su trabajo ya vive, adaptado,
en esta rama.
...
plantilla de validación de Bancoldex" — es decir, F6 y F7 convergen en la
