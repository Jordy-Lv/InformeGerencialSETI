...
| F7 | **F7a completada** (adaptador Aranda + perfil de datos, rama `f7/bancoldex-aranda-perfil` desde el cierre de F5). `perfiles/base.js` y `perfiles/bancoldex.js` declarados; `adaptarArandaACanonico()`/`cargarCasosAranda()` nuevos, sin tocar `cargarGlpi()`. El 05/08/2026 se verificó contra el export real de junio-2026 (`Bancoldex/`, no versionado): 72 casos, agregados por tipo (Incidente-Monitoreo 33, Requerimiento 32, Tarea 5, Incidente 2) y por motor (Oracle 52, SQL Server 19, WebLogic 1), y SLA (71 cumple / 1 no cumple) coinciden exactamente con `Bancoldex/reporte-bancoldex-2026-07-02.pdf`. Se cargó `?perfil=bancoldex` en un navegador real sin errores de consola; Acción Fiduciaria se verificó sin cambios en el mismo navegador. **Pendiente F7b:** tarjeta visual de casos (4 categorías), lectores de consolidado (indicadores de cabecera de dos filas, backups por BD, línea base con ambiente, disponibilidad por motor), y coordinar el orden de merge con F6 antes de llevar la rama a `main`. Ver `docs/2026-08-05-f7-bancoldex-aranda.md`. |
| F8–F11 | Pendientes |
