...
| F1 | **En curso — PR #12 (`f1/perfil-accion-fiduciaria`).** Primer incremento: perfil de datos, `resolverPerfil()`, `fusionarProfundo` global, `REPORTE.cliente`, filtros y claves de almacén migrados; la resolución del conflicto combina correctamente claves nuevas/viejas con herencia de bolsa entre periodos. El export ya transporta el perfil resuelto sin depender de `perfiles/`, y la capacidad `perfil-cliente` tiene change, spec y pruebas. **Pendiente:** hidratar los ~20 literales de interfaz reservados para F1 parte 2 y ejecutar A/B con exportaciones reales completas; hasta obtener cero diferencias la PR no se fusiona. |
| F2–F11 | Pendientes |
