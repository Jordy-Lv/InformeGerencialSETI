...
| F6 | **En progreso en otra rama** (`codex/f6-perfil-novaventa`), fuera del alcance de esta rama — ver la nota de coordinación en `openspec/changes/2026-08-05-f7-bancoldex-aranda/proposal.md`. |
| F7 | **F7a y F7b completadas** (adaptador Aranda + perfil + integración real, rama `f7/bancoldex-aranda-perfil` desde el cierre de F5). `perfiles/base.js`/`bancoldex.js` declarados con datos verificados contra el PDF y el consolidado reales; `adaptarArandaACanonico()`/`cargarCasosOGlpi()` enrutan casos sin tocar `cargarGlpi()`; `cargarIndicadores()`/`cargarBackups()` generalizados por perfil (mismo default de AF); `cargarDisponibilidadTabla()` nuevo; `PERFIL.lineaBase`/`presentarTarjetaPerfil()` cierran el "gap preexistente" de F7a. El 05/08/2026 se cargaron en navegador real los dos archivos de junio-2026: indicadores (3/3), backups (11 BD, 100%) y casos (72, agregados por tipo/motor/SLA) coinciden con el PDF de referencia; disponibilidad por motor queda bloqueada por un hallazgo real (la hoja `Disponibilidad Real` del cliente no tiene columna para jun-26, no es un bug). Acción Fiduciaria verificada sin cambios en el mismo navegador. **Pendiente:** tarjeta visual de casos (4 categorías), reconciliar con `perfiles/base.js`/`bancoldex.js` agregados también al directorio principal (fuera de esta rama, F6 sigue activo ahí), y coordinar el orden de merge con F6 antes de llevar la rama a `main`. Ver `docs/2026-08-05-f7-bancoldex-aranda.md`. |
| F8–F11 | Pendientes |
